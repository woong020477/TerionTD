...
/// <summary>서버가 전달한 방 상태로 목록·상세·참가 패널을 갱신하는 로비 UI 진입점입니다.</summary>
public class LobbyManager : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject roomFilterPanel;
    [SerializeField]
    ...
    [SerializeField]
    private TMP_Text outputRoomNameText_RI;
    [SerializeField]
    private TMP_Text outputHostNameText_RI;
    [SerializeField]
    private TMP_Text outputDifficultyText_RI;
    private int lastClickedRoomId = -1; // 마지막으로 클릭한 RoomId 저장
    private int selectedRoomInfoId = -1; // 우측 정보 패널이 표시 중인 RoomId
    private float lastClickTime = 0f; // 마지막 클릭 시간
    private float doubleClickThreshold = 0.3f; // 더블클릭 인식 시간(초)
    [Header("RoomPanel UI - RoomJoin")]
    [SerializeField]
    private GameObject roomJoinPanel;
    [SerializeField]
    private TMP_Text outputRoomNameText_RJ;
    [SerializeField]
    private TMP_Text outputHostNameText_RJ;
    [SerializeField]
    private TMP_Text outputDifficultyText_RJ;
    [Header("RoomPanel UI - PlayerList")]
    [SerializeField]
    private GameObject playerSlotPrefab; // 플레이어 슬롯 UI 프리팹
    [SerializeField]
    private Transform playerListContent_RJ; // RoomJoin용
    [SerializeField]
    private Transform playerListContent_RI; // RoomInfo용
    private int currentRoomId;
    private int hostUserId;
    [Header("RoomFilterPanel UI")]
    [SerializeField]
    private TMP_InputField hostUsernameInputField_RF;
    [SerializeField]
    private TMP_Dropdown difficultyDropdown_RF;
    [SerializeField]
    private TMP_InputField roomNameInputField_RF;
    private List<GameObject> currentRoomButtons = new List<GameObject>(); // 현재 생성된 RoomButton 캐싱
    [Header("CreateRoomPanel UI")]
    [SerializeField]
    private TMP_InputField roomNameInputField_CR;
    [SerializeField]
    private TMP_Dropdown difficultyDropdown_CR;
    [SerializeField]
    ...
    [Header("No Panel UI")]
    [SerializeField]
    private TMP_Text PingStatusText;
    [SerializeField]
    private TMP_Text lobbyStatusTextPrefab;
    [SerializeField]
    private Transform statusTextParent;
    ...
    /// <summary>서버 상태 이벤트를 구독하여 열린 로비 UI가 최신 응답을 받도록 합니다.</summary>
    private void OnEnable()
    {
        ...
        AuthManager.OnManualRoomListRefreshCompleted += HandleManualRoomListRefreshCompleted;
    }

    /// <summary>씬 이동과 재진입 시 이벤트가 중복 처리되지 않도록 구독을 해제합니다.</summary>
    ...
    {
        // 방 액션 결과 처리
        if (command == "exit-room-result")
        {
            CloseJoinedRoomPanel();
        }
    ...
    {
        CloseRoomInfoPanelIfSelected(roomId);
    ...
    public void RoomPanel_OnRefreshRoomListClick()
    {
        ...
        AuthManager.Instance.SendRequestRoomList(true);
    }

    private void HandleManualRoomListRefreshCompleted(string message)
    {
        PrintStatusText(message, Color.green);
    }

    private void UpdateRoomInfoPanel(string json)
    {
        var data = JsonUtility.FromJson<RoomUpdateData>(json);
        if (data == null || data.RoomId <= 0 || data.Players == null)
        {
            CloseRoomInfoPanelIfSelected(selectedRoomInfoId);
            return;
        }

        selectedRoomInfoId = data.RoomId;
        RenderRoomInfoPanel(data);
    }

    private void RenderRoomInfoPanel(RoomUpdateData data)
    {
        ...
        foreach (var p in data.Players)
        {
            ...
            if (img != null)
            {
                ...
                {
                    case 1:
                        ColorUtility.TryParseHtmlString("#CA3038", out slotColor);
                        break;
                    case 2:
                        ColorUtility.TryParseHtmlString("#3D79A7", out slotColor);
                        break;
                    case 3:
                        ColorUtility.TryParseHtmlString("#508E34", out slotColor);
                        break;
                    case 4:
                        ColorUtility.TryParseHtmlString("#8544AD", out slotColor);
                        break;
                }
            ...
            foreach (var t in texts)
            {
                if (t.name == "PlayerNameText")
                    t.text = p.IsClosed ? "Close" : (p.UserId == 0 ? "Open" : $"<gradient={p.ColorGradient}>{p.Username}</gradient>");
                else
    ...
    /// <summary>서버의 슬롯·방장 상태를 참가 중인 방 UI에 반영합니다.</summary>
    private void UpdateRoomInfoUI(string json)
    {
        var data = JsonUtility.FromJson<RoomUpdateData>(json);
        if (data == null || data.RoomId <= 0 || data.Players == null)
            return;
        if (selectedRoomInfoId == data.RoomId && roomInfoPanel.activeSelf)
            RenderRoomInfoPanel(data);
        bool localPlayerIsMember = data.Players.Any(player => player.UserId == AuthManager.loggedInUserId);
        if (!localPlayerIsMember)
        {
    ...
    private void CreatePlayerSlot(PlayerData player)
    {
        ...
        foreach (var t in texts)
        {
            ...
            {
                case "PlayerNameText":
                    t.text = player.IsClosed ? "Close" : (player.UserId == 0 ? "Open" : $"<gradient={player.ColorGradient}>{player.Username}</gradient>");
                    break;
            }
        }
        ...
        foreach (var btn in buttons)
        {
            ...
            {
                case "UpButton":
                    btn.onClick.AddListener(() => RoomJoin_OnMoveSlotClick(slotNumber, slotNumber - 1 < 1 ? 4 : slotNumber - 1));
                    btn.gameObject.SetActive(isHostUser && player.UserId != 0);
                    break;
                case "DownButton":
                    btn.onClick.AddListener(() => RoomJoin_OnMoveSlotClick(slotNumber, slotNumber + 1 > 4 ? 1 : slotNumber + 1));
                    btn.gameObject.SetActive(isHostUser && player.UserId != 0);
                    break;
                case "HostButton":
    ...
    /// <summary>강퇴 응답에 맞춰 참가 패널과 선택 방 상태를 함께 정리합니다.</summary>
    public void HandleKickedFromRoom(int roomId, string message)
    {
        if (currentRoomId != roomId)
            return;
        CloseRoomInfoPanelIfSelected(roomId);
        CloseJoinedRoomPanel();
        PrintStatusText(message, Color.red);
        AuthManager.Instance?.SendRequestRoomList();
    }

    /// <summary>현재 방 번호로 게임 시작을 요청하며, 실제 시작 권한은 서버에서 다시 검증합니다.</summary>
    public void OnClickStartGame()
    {
        SoundManager.Instance.PlaySFX(SoundKey.UI_Click);
        int roomId = currentRoomId;
        string json = $"{{\"Command\":\"game-start\",\"RoomId\":{roomId}}}";
        AuthManager.Instance.SendServerMessage(json);
    }
    ...
    public void RoomFilter_OnRoomFilterApplyClick()
    {
        ...
        PrintStatusText("방 목록을 갱신했습니다.", Color.green);
    }
    ...
    /// <summary>최신 방 목록으로 버튼을 재구성하고 사라진 방의 상세 패널을 닫습니다.</summary>
    private void UpdateRoomListUI(string json)
    {
        try
        {
            ...
            HashSet<int> visibleRoomIds = new HashSet<int>();
            foreach (var room in roomData.Rooms)
            {
                ...
                btnObj.transform.Find("RoomStatusText").GetComponent<TMP_Text>().text = $"Join Player [{room.CurrentPlayers}/{room.MaxPlayers}]\nHost : <gradient={room.ColorGradient}>{room.Host}</gradient>";
                // 태그 데이터 보관 (필터링용)
            ...
            if (selectedRoomInfoId > 0 && !visibleRoomIds.Contains(selectedRoomInfoId))
                CloseRoomInfoPanelIfSelected(selectedRoomInfoId);
        }
        catch (Exception ex)
        {
    ...
    private void HandleRoomButtonClick(int roomId, string roomName, string hostName, string difficulty)
    {
        ...
        if (lastClickedRoomId == roomId && timeSinceLastClick <= doubleClickThreshold)
        {
            ...
            selectedRoomInfoId = -1;
        }
        else
        {
            lastClickedRoomId = roomId;
            selectedRoomInfoId = roomId;
    ...
    /// <summary>방 참가 상태의 UI를 닫고 다음 입장에 남지 않아야 할 참조를 지웁니다.</summary>
    private void CloseJoinedRoomPanel()
    {
        int closedRoomId = currentRoomId;
        currentRoomId = 0;
        hostUserId = 0;
        roomJoinPanel.SetActive(false);
        createRoomButton.SetActive(true);
        CloseRoomInfoPanelIfSelected(closedRoomId);
    }

    private void CloseRoomInfoPanelIfSelected(int roomId)
    {
        if (roomId <= 0 || selectedRoomInfoId != roomId)
            return;
        roomInfoPanel.SetActive(false);
        selectedRoomInfoId = -1;
        if (lastClickedRoomId == roomId)
            lastClickedRoomId = -1;
    }

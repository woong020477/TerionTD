...
/// <summary>로컬 플레이어 HUD와 선택 패널을 관리합니다. 원격 상태 갱신이 로컬 선택을 바꾸지 않도록 합니다.</summary>
public class UIManager : MonoBehaviour
{
 ...
 = new Dictionary<int, Dictionary<TowerType, int>>();
    ...
    [SerializeField]
    private TMP_Text EnemyAttribute;
    [SerializeField]
    private TMP_Text EnemyArmor;
    [SerializeField]
    private TMP_Text AttributeText; // 속성 설명 텍스트
    [Header("건물 정보 UI")]
    ...
    [Header("리더보드 버튼")]
    [SerializeField]
    private GameObject LeaderBoard_Button_UpButton;
    [SerializeField]
    private GameObject LeaderBoard_Button_DownButton;
    [Header("타워 업그레이드 버튼")]
    [SerializeField]
    private GameObject FlameThrowerTowerButton;
    [SerializeField]
    private GameObject LaserToweButton;
    [SerializeField]
    private GameObject MachineGunTowerButton;
    [SerializeField]
    private GameObject MultipleRocketLuncherTowerButton;
    [SerializeField]
    private GameObject RocketTowerButton;
    [Header("타워 연구소 버튼")]
    [SerializeField]
    private GameObject FlameThrowerTower_Lab_Button;
    [SerializeField]
    private GameObject LaserTower_Lab_Button;
    [SerializeField]
    private GameObject MachineGunTower_Lab_Button;
    [SerializeField]
    private GameObject MultipleRocketLuncherTower_Lab_Button;
    [SerializeField]
    private GameObject RocketTower_Lab_Button;
    [Header("배속 버튼")]
    [SerializeField]
    private TextMeshProUGUI xButtonText;
    [SerializeField]
    ...
    [HideInInspector]
    public PlayerController owner; // 현재 타워 연구소(플레이어 컨트롤러)
    ...
    public int LocalPlayerIndex => owner ? owner.PlayerIndex : -1;

    private bool IsReady() => owner != null && owner.IsLocalPlayer && LocalPlayerIndex >= 0;
    private void Awake()
    {
        ...
        if (attackRangeLayer >= 0)
            worldClickMask &= ~(1 << attackRangeLayer);
    }
    ...
    /// <summary>선택된 타워의 소유자를 확인해 이동 버튼과 실제 동작이 같은 권한 규칙을 따르게 합니다.</summary>
    private bool CanMoveCurrentTower()
    {
        return currentTower != null && currentTower.Owner != null && currentTower.Owner.IsLocalPlayer;
    }
    ...
    private void MoveSelectedTower()
    {
        ...
        if (currentTower == null)
            return;
        if (currentTower.Owner == null || !currentTower.Owner.IsLocalPlayer)
            return;
        owner?.CancelBuildModeIfActive();
        if (!BuildingSystem.Instance.BeginGrid(BuildingSystem.GridOwner.Move, this, owner))
        {
    ...
    private IEnumerator MoveSelectedTowerRoutine(TowerBase baseComp)
    {
        ...
        while (isMoveMode)
        {
            ...
            if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
            {
                ...
                if (BuildingSystem.Instance.CanPlaceFor(baseComp.Owner))
                {
                    ...
                    if (tc)
                        tc.transform.position = newTowerPos;
                    // 네트워크 통보
                    UDPClient.Instance?.SendUDP(JsonUtility.ToJson(new TowerMoveMessage { ownerIndex = owner ? owner.PlayerIndex : -1, baseNetId = baseComp.NetId, basePosition = newBasePos, hasTower = (tc != null), towerPosition = newTowerPos }), "TOWER_MOVE");
    ...
    public void UpdateGoldUI(int index)
    {
        ...
        if (playerSet.controller == null || playerSet.GoldText == null)
            return;
    ...
    {
        // 패널이 꺼져 있으면 검사 불필요
        if (!BuildingStatusPanel || !BuildingStatusPanel.activeSelf)
            return;
        ...
        if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
        {
            ...
            else
            {
                ...
                ToggleSelectTowerPanel(false);
                if (owner != null)
                    SetUpgradeUI(owner.PlayerIndex, false);
            }
        }
    }

    public void ToggleSelectTowerPanel(bool selectTowerFlag)
    {
        SelectTowerPanel.SetActive(selectTowerFlag);
    }

    // 타워 생성 버튼 클릭 이벤트 핸들러
    public void OnCreateTowerButtonClicked(TowerType TowerType)
    {
    ...
    private void UpdateEnemyHPGradient(Enemy currentEnemy)
    {
        // 현재 HP 비율 계산 (0 ~ 1)
    ...
    public void UpdateBuildingStatus(TowerController tower, TowerBase baseInfo = null)
    {
        BuildingStatusPanel.SetActive(true);
        currentTower = baseInfo;
        if (moveButton != null)
            moveButton.interactable = CanMoveCurrentTower();
        // 타워 이름 출력
    ...
    /// <summary>현재 보고 있는 타워만 갱신해 원격 업그레이드가 선택 창을 새로 열지 않도록 합니다.</summary>
    public void RefreshBuildingStatusIfSelected(TowerController tower)
    {
        if (tower == null || currentTower == null)
            return;
        if (tower.GetComponentInParent<TowerBase>() != currentTower)
            return;
        UpdateBuildingStatus(tower, currentTower);
    }
    ...
    public void UpdateBuildingStatus_Base(TowerBase baseInfo)
    {
        if (baseInfo == null)
        {
            BuildingStatusPanel.SetActive(false);
    ...
    public void IncreaseTowerBuildCount(TowerType type)
    {
    ...
    public float GetBuildCost(TowerType type)
    {
        if (!towerBuildCounts.ContainsKey(type))
            towerBuildCounts[type] = 0;
    ...
    private void UpdateTowerPriceText(GameObject buttonObj, TowerType type)
    {
        if (buttonObj == null)
            return;
    ...
    public int GetLabLevel(int playerIndex, TowerType towerType)
    {
        if (!playerLabUpgradeCounts.TryGetValue(playerIndex, out var map))
        {
            ...
            foreach (TowerType tt in System.Enum.GetValues(typeof(TowerType)))
                map[tt] = 1;
    ...
    {
        var _ = GetLabLevel(playerIndex, t);
    ...
    public void TryUpgradeLab(int index, TowerType towerType)
    {
        ...
        foreach (var tower in TowerController.Towers)
        {
            ...
            if (tower.Owner == player && tower.towerType == towerType)
            {
                ...
                RefreshBuildingStatusIfSelected(tower);
            }
        }

        UpdateLabButtons();
        // 3) 네트워크 브로드캐스트(모든 클라가 UI/타워 상태 맞춤)
        UDPClient.Instance?.SendUDP(JsonUtility.ToJson(new TowerLabUpgradeMessage { ownerIndex = index, towerType = towerType, level = newLevel }), "TOWER_LAB_UPGRADE");
    }
    ...
    private void UpdateTowerLabButton(GameObject buttonObj, TowerType towerType)
    {
        ...
        if (priceText)
            priceText.text = (nextData != null) ? $"{cost}G" : "MAX";
        if (countText)
            countText.text = $"{currentLevel}회";
    }
    ...
    public void ShowEnemyInfoUI(Enemy enemy)
    {
        currentEnemy = enemy;
        ...
        EnemyArmor.text = $"{enemy.Armor}%";
        EnemyName.text = enemy.EnemyName;
    }

    public void ShutdownEnemyInfoUI()
    {
        EnemyInfoUI.SetActive(false);
        AttributeInfoPanel.SetActive(false);
    }
    ...
    public void ShutdownLeaderBoardUI()
    {

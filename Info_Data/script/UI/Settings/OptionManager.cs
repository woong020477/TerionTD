...
public struct ResOption
{
    ...
    public ResOption(int w, int h, bool winOnly)
    {
        width = w;
        height = h;
        windowedOnly = winOnly;
    }
...
/// <summary>옵션 UI 입력을 해상도·화면 모드·사운드 설정에 반영합니다.</summary>
public class OptionManager : MonoBehaviour
{
    ...
    [Header("Canvas & 그룹")]
    [SerializeField]
    private CanvasGroup canvasGroup; // 열기/닫기 시 α 즉시 변경 대상
    [Header("사운드 UI")]
    [SerializeField]
    private Slider bgmSlider; // 0.0~1.0
    [SerializeField]
    private Slider sfxSlider; // 0.0~1.0
    [Header("해상도/전체화면 UI")]
    [SerializeField]
    ...
    [SerializeField]
    private string bgmParam = "BGM"; // AudioMixer 노출 파라미터명
    [SerializeField]
    private string sfxParam = "SFX"; // AudioMixer 노출 파라미터명
    ...
    {
        if (!audioMixer || string.IsNullOrEmpty(param))
            return;
        // 0 -> -80dB, 1 -> 0dB

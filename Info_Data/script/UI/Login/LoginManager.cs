...
/// <summary>로그인 화면의 입력과 패널 전환을 담당하며 인증 요청은 AuthManager에 위임합니다.</summary>
public class LoginManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        tabOrderInputFields = new TMP_InputField[]
        {
            ...
            emailInputField,
            passwordInputField,
            passwordInputField_F,
            usernameInputField_F,
            emailInputField_F,
            usernameInputField_C,
            emailInputField_C,
        ...
        if (Input.GetKeyDown(KeyCode.Tab))
        {
            ...
            for (int i = 0; i < tabOrderInputFields.Length; i++)
            {
                if (currentObj == tabOrderInputFields[i].gameObject)
                {
                    ...
                    {
                        if (tabOrderInputFields[nextIndex].gameObject.activeInHierarchy && tabOrderInputFields[nextIndex].interactable)
                        {

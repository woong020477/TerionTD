...
public struct SoundEntry
{
    ...
    [Range(0f, 1f)]
    public float volume; // 개별 기본 볼륨 (옵션 볼륨과 곱해짐)
    public float VolumeOrDefault => (volume <= 0f ? 1f : volume);
}

/// <summary>오디오 클립 조회와 BGM·효과음 재생 및 볼륨을 관리합니다.</summary>
public class SoundManager : MonoBehaviour
{
    ...
    [SerializeField]
    private string bgmParam = "BGM"; // OptionManager와 동일 노출 파라미터명
    [SerializeField]
    private string sfxParam = "SFX";
    [Header("클립 매핑")]
    [SerializeField]
    private List<SoundEntry> clips = new();
    [SerializeField]
    private AudioSource bgmSource; // BGM 전용(루프/크로스페이드)
    [SerializeField]
    private AudioSource sfxSource; // OneShot 전용(겹쳐 재생 가능)
    [Header("기본 BGM")]
    [SerializeField]
    private SoundKey startupBgm = SoundKey.BGM;
    [SerializeField]
    private bool playBgmOnInit = true;
    // 내부
    ...
    {
        if (!dict.TryGetValue(key, out var e) || !e.clip)
            return;
        if (!audioEnabled)
            return;
        bgmSource.loop = e.isBgm || e.loop;
        if (bgmFadeCo != null)
            StopCoroutine(bgmFadeCo);
    ...
    float GetBgmScalar()
    {
        if (!mixer || string.IsNullOrEmpty(bgmParam))
            return OptionDataManager.Instance?.Data.bgmVolume ?? 1f;
        if (mixer.GetFloat(bgmParam, out var dB))
    ...
    float GetSfxScalar()
    {
        if (!mixer || string.IsNullOrEmpty(sfxParam))
            return OptionDataManager.Instance?.Data.sfxVolume ?? 1f;
        if (mixer.GetFloat(sfxParam, out var dB))
            return Mathf.Pow(10f, dB / 20f);
        return 1f;
    }
}
